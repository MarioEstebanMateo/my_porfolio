import { Heart } from "lucide-react";
import { useLanguage } from "../context/LanguageContext";
import { translations } from "../translations";

export const Footer = () => {
  const year = new Date().getFullYear();
  const { language } = useLanguage();
  const t = translations[language].footer;

  return (
    <footer className="relative overflow-hidden bg-gradient-to-r from-primary-600 to-accent-600 text-white">
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-20"></div>
      <div className="relative z-10 text-center py-8 px-4 font-opensans">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 text-sm sm:text-base">
          <span>&copy; {year} {t.allRightsReserved} | {t.builtWith}</span>
          <Heart className="w-4 h-4 fill-red-400 text-red-400 animate-pulse" />
          <span>{t.by} <span className="font-bold">Mario Esteban Mateo</span></span>
        </div>
      </div>
    </footer>
  );
};
